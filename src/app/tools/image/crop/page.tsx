...
const guide = {
  ...
  sections: [
    ...
    {
      heading: "관공서·시험별 증명사진 픽셀과 배경 규격",
      paragraphs: [
        "규격에 맞춰 자르려면 목표 픽셀과 배경색을 먼저 알아야 합니다. 관공서·시험용은 흰색 배경에 정해진 픽셀을 요구하는 반면, 취업 사이트는 크기를 따로 정하지 않는 경우가 많습니다. 아래 규격대로 크롭 영역과 출력 크기를 잡으면 접수 화면에서 되돌려받을 일이 줄어듭니다.",
        "표의 픽셀은 각 기관이 안내하는 값으로, '정확한 크기로 출력'에 그대로 넣어 쓸 수 있습니다. 배경이 흰색으로 지정된 용도는 촬영 단계에서 흰 벽이나 흰 배경을 두고 찍는 편이 깔끔합니다. 규격은 갱신될 수 있으니 제출 전 기관 공지도 함께 살펴보세요.",
      ],
      table: {
        headers: ["용도", "픽셀 규격", "배경", "출처(기관)"],
        rows: [
          ["토익 응시원서", "115×150px", "흰색", "YBM(toeic.co.kr)"],
          ["공무원 원서접수", "137×177px", "흰색", "사이버국가고시(gosi.kr)"],
          ["운전면허 신청", "350×450px", "흰색", "도로교통공단(koroad)"],
          ["여권 신청", "413×531px", "흰색", "외교부(passport.go.kr)"],
          ["잡코리아 이력서", "자유", "지정 없음", "잡코리아(jobkorea)"],
          ["사람인 이력서", "자유", "지정 없음", "사람인(saramin)"],
        ],
        caption: "각 기관 증명사진 규격 안내 기준. 배경 '지정 없음'은 별도 규정이 없다는 뜻입니다.",
      },
    },
    {
      heading: "AI 보정 금지 규정과 크롭",
      paragraphs: [
        "여권·운전면허 온라인 신청 안내에는 편집 프로그램이나 필터, AI로 얼굴을 합성·가공한 사진은 받지 않는다는 규정이 있습니다. 이 도구가 하는 일은 사진에서 선택한 사각형 영역의 픽셀을 그대로 오려내는 것뿐이라, 눈·코·윤곽이나 피부, 배경을 새로 만들어내거나 손대지 않습니다.",
        "즉 크롭은 원본 얼굴을 그대로 둔 채 테두리만 잘라내므로 보정·합성을 금지하는 조항과 부딪힐 소지가 적습니다. 다만 얼굴 크기나 표정 같은 세부 요건은 접수처마다 다르니, 규정 충족 여부는 마지막에 해당 기관 안내로 한 번 더 확인하시길 권합니다.",
      ],
    },
    {
