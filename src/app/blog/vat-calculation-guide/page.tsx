    ...
    <>
      ...
      <main className="mx-auto max-w-3xl px-4 py-10">
        ...
        <div className="prose prose-gray max-w-none">
          <p>부가세 계산은 금액이 부가세 별도인지 포함인지부터 나누면 단순해집니다. 일반과세 기준 부가가치세율은 10%이므로 공급가액에 1.1을 곱하면 합계금액이 되고, 이미 부가세가 포함된 합계금액은 1.1로 나누면 공급가액이 됩니다.</p>
          ...
          <h2>부가세는 어떤 금액에 붙나요?</h2>
          <p>부가세는 재화나 용역의 공급가액을 기준으로 계산합니다. 일반과세 기준에서는 공급가액의 10%가 부가세이고, 소비자가 최종적으로 보는 합계금액은 공급가액과 부가세를 더한 값입니다. 그래서 견적서에 1,000,000원이라고 적혀 있어도 그 금액이 공급가액인지, 부가세까지 들어간 총액인지에 따라 결과가 달라집니다.</p>
          <p>실무에서는 “부가세 별도”라는 표현이 있으면 공급가액을 먼저 적고 나중에 10%를 더합니다. 반대로 카드 영수증이나 온라인 결제 화면처럼 이미 납부할 총액이 보이는 경우에는 그 안에 들어 있는 공급가액과 부가세를 나누어 보는 방식이 필요합니다.</p>
          ...
          <h2>부가세 별도와 포함은 왜 자주 헷갈리나요?</h2>
          <p>같은 숫자라도 기준 금액이 다르면 실제 청구액이 달라지기 때문입니다. “100만원”이라는 말이 공급가액을 뜻하면 최종 청구액은 110만원이고, 합계금액을 뜻하면 공급가액은 약 90만9천원이 됩니다. 계약서, 견적서, 세금계산서에서는 이 차이가 금액 오해로 이어질 수 있습니다.</p>
          <p>가장 안전한 확인 순서는 세 가지입니다. 첫째, 상대방이 말한 금액이 별도인지 포함인지 묻습니다. 둘째, 공급가액과 부가세를 줄로 나누어 적습니다. 셋째, 합계금액이 실제 결제 금액과 같은지 확인합니다. 이 순서만 지켜도 대부분의 단순 계산 오류는 줄어듭니다.</p>
          <h2>사업자가 내는 부가세는 어떻게 이해하면 되나요?</h2>
          <p>사업자 입장에서는 매출세액에서 매입세액을 뺀 금액이 납부세액의 기본 구조입니다. 매출할 때 받은 부가세 전액을 그대로 비용처럼 보는 것이 아니라, 사업을 위해 매입하면서 부담한 부가세를 함께 고려합니다. 다만 매입세액 공제 가능 여부와 신고 방식은 상황에 따라 달라질 수 있습니다.</p>
          <p>간이과세자는 계산 구조가 일반과세자와 다릅니다. 업종별 부가가치율을 곱해 세부담을 산정하므로 일반과세의 공급가액 10% 방식만으로 최종 부담을 단정하기 어렵습니다. 이 글의 공식은 일반과세 기준의 공급가액, 부가세, 합계금액 관계를 이해하기 위한 기본식으로 보면 됩니다.</p>
          <h2>견적서에 적을 때는 무엇을 확인해야 하나요?</h2>
          <p>견적서에는 공급가액, 부가세, 합계금액을 분리해 적는 편이 분쟁을 줄입니다. 한 줄에 총액만 적으면 상대방은 부가세 포함 금액으로 이해할 수 있고, 작성자는 별도 금액으로 생각할 수 있습니다. 특히 반복 거래나 용역 계약에서는 처음 합의한 기준 금액이 이후 청구서와 세금계산서까지 이어지므로 표현을 명확히 남기는 것이 좋습니다.</p>
          <p>계산 후에는 합계금액이 실제 입금 또는 결제 요청 금액과 일치하는지도 확인해야 합니다. 공급가액을 반올림했거나 원 단위 절사 처리를 했다면 부가세와 합계가 1원 단위로 달라질 수 있습니다. 이런 차이는 계산 원칙을 맞춘 뒤 거래처와 표시 방식을 정하면 됩니다.</p>
          <h2>FAQ</h2>
          <section><h3>공급가액과 합계금액 중 무엇을 먼저 확인해야 하나요?</h3><p>부가세 포함 여부를 먼저 확인해야 합니다. 포함 금액이면 합계를 1.1로 나누고, 별도 금액이면 공급가액에 10%를 더하면 됩니다.</p></section>
          <section><h3>합계금액에서 부가세만 바로 구할 수 있나요?</h3><p>일반과세 기준에서는 합계금액에 10/110을 곱하면 됩니다. 합계 550,000원이라면 부가세는 50,000원입니다.</p></section>
          <section><h3>매출 부가세 전부가 납부세액인가요?</h3><p>보통은 매출세액에서 매입세액을 뺀 값이 납부세액의 출발점입니다. 실제 신고 결과는 공제 가능 여부와 과세 유형에 따라 달라질 수 있습니다.</p></section>
          <section><h3>간이과세도 같은 방식인가요?</h3><p>간이과세는 업종별 부가가치율을 반영하므로 일반과세와 세부담 구조가 다릅니다. 단순 합계 역산에는 일반과세 공식을 쓰되 신고 판단은 별도로 확인해야 합니다.</p></section>
        </div>
