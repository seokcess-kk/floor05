...
| ✅ | 워터마크 | 워터마크 넣기, 사진 워터마크 | 완료(2026-06). 텍스트/로고·9위치·전체반복·일괄. 블로그 2편(넣는 법/도용 방지). 워터마크 **제거**는 AdSense 저작권 우회 정책·AI 모델 부담으로 보류 |
| 다음 | 모자이크/블러 | 얼굴·개인정보 가리기 | 드래그 영역(크롭 패턴 재사용) |
| 이후 | 사진 회전/반전 | 사진 회전 | crop의 applyTransformations 재사용, 경량 |
| 전략 1순위(검토) | **PDF Room** | PDF 합치기, 이미지 PDF 변환, PDF JPG 변환 | 대형 수요+고CPC+서버 미전송 차별화. ToolCategory에 'pdf' 추가 + pdf-lib/pdf.js 페이지별 동적 로드 |
| 검토 | 9분할/긴 이미지 분할 · 4대보험료 계산기 | 인스타 9분할 / 4대보험료 | merge 역연산 · rates.ts 재사용 |
